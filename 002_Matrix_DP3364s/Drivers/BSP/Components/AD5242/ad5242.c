...
{ 
	...
	hi2c_ee.i2cx = AD5242_I2C;

  /* reset i2c peripheral */
  i2c_reset(hi2c_ee.i2cx);

  /* i2c peripheral initialization */
  i2c_lowlevel_init(&hi2c_ee, AD5242_I2C_CLKCTRL_200K);
...
{
  crm_periph_clock_enable(AD5242_I2C_CLK, FALSE);
	return AD5242_OK;
}
...
uint8_t AD5242_Write(uint8_t devAdr, uint8_t *pTxData)
{
	...
	data[0] = RDAC_A;
	data[1] = *pTxData++;
	res = i2c_master_transmit(&hi2c_ee, devAdr, data, 2, AD5242_TIMEOUT);
	if (res != AD5242_OK)
		return res;
...
uint8_t AD5242_Read(uint8_t devAdr, uint8_t *pRxData)
{
	...
	//RADC_A
	data = RDAC_A;
	res = i2c_master_transmit(&hi2c_ee, devAdr, &data, 1, AD5242_TIMEOUT);
//	res = HAL_I2C_Master_Transmit(&hi2c4, devAdr, &data, 1, AD5242_TIMEOUT);
	if (res != AD5242_OK)
		return res;
	res = i2c_master_receive(&hi2c_ee, devAdr, pRxData, 1, AD5242_TIMEOUT);
//	res = HAL_I2C_Master_Receive(&hi2c4, devAdr, pRxData, 1, AD5242_TIMEOUT);
	if (res != AD5242_OK)
		return res;
...
static void i2c_lowlevel_init(i2c_handle_type* hi2c, uint32_t i2c_speed)
{
  ...
  crm_periph_clock_enable(AD5242_I2C_CLK, TRUE);
  crm_periph_clock_enable(AD5242_I2C_SCL_GPIO_CLK, TRUE);
  crm_periph_clock_enable(AD5242_I2C_SDA_GPIO_CLK, TRUE);

  /* gpio configuration */
  gpio_pin_mux_config(AD5242_I2C_SCL_GPIO_PORT, AD5242_I2C_SCL_GPIO_PINS_SOURCE, AD5242_I2C_SCL_GPIO_MUX);

  gpio_pin_mux_config(AD5242_I2C_SDA_GPIO_PORT, AD5242_I2C_SDA_GPIO_PINS_SOURCE, AD5242_I2C_SDA_GPIO_MUX);
  ...
  gpio_init_structure.gpio_pins           = AD5242_I2C_SCL_GPIO_PIN;
  gpio_init(AD5242_I2C_SCL_GPIO_PORT, &gpio_init_structure);

  /* configure i2c pins: sda */
  gpio_init_structure.gpio_pins           = AD5242_I2C_SDA_GPIO_PIN;
  gpio_init(AD5242_I2C_SDA_GPIO_PORT, &gpio_init_structure);
