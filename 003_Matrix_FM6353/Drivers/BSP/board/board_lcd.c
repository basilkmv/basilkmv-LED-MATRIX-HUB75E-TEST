...
void xmc_init(void)
{
  ...
  gpio_pin_mux_config(LCD_A20_GPIO_PORT,  LCD_A20_GPIO_PINS_SOURCE,  LCD_A20_GPIO_MUX);
  ...
  gpio_init_struct.gpio_pins = LCD_A20_GPIO_PIN;
  //gpio_init_struct.gpio_pull = GPIO_PULL_UP;
  gpio_init(LCD_A20_GPIO_PORT,  &gpio_init_struct);
  ...
  gpio_init_struct.gpio_pins = LCD_ON_GPIO_PIN;
  gpio_init(LCD_ON_GPIO_PORT,  &gpio_init_struct);
  ...
  rw_timing_struct.data_setup_time = 0x5;
  ...
  w_timing_struct.data_setup_time = 0x5;
...
//  uint32_t total_point = lcddev.width * lcddev.height;
  uint32_t total_point = 480 * 272;

//  lcd_windows_set(0, 0, lcddev.width - 1, lcddev.height - 1);
  lcd_windows_set(0, 0, 480 - 1, 272 - 1);
...
void lcd_dma_init(void){
  ...
  crm_periph_clock_enable(LCD_DMA_CLK, TRUE);
  dma_reset(LCD_DMA_CHANNEL);
  ...
  nvic_irq_enable(LCD_DMA_IRQn, LCD_DMA1_Channel3_IT, 0);
  dma_interrupt_enable(LCD_DMA_CHANNEL,DMA_FDT_INT,TRUE);
}
...
void lcd_init(lcd_display_type direction)
{
  ...
  while(!initOk){  
    ...
    //Set the MN of PLL 
    lcd_reg_write(0xe2);   
    lcd_data_write(0x1d);//multiplier 0x1d
    lcd_data_write(0x02);//devider   
    lcd_data_write(0x54);//0x04 
  
    lcd_reg_write(SET_PLL);     
    lcd_data_write(0x01); 
    delay_ms(10); 
    lcd_reg_write(SET_PLL);    
    lcd_data_write(0x03); 
    ...
    lcd_reg_write(SOFT_RESET); 
    delay_ms(10);//5 ms 
  
    lcd_reg_write(0xb0);  
    lcd_data_write(0x20); //24-bit 
    lcd_data_write(0x80);    
    lcd_data_write(0x01); //Horisontal size   
    lcd_data_write(0xdf);       
    lcd_data_write(0x01); //Vertical Size  
    lcd_data_write(0x0f);   
    lcd_data_write(0x00);//RGB   
	
    lcd_command_read(GET_LCD_MODE,readData,7);
    if (readData[1] == 0x80)
      initOk = 1;
    else
      BSP_LED_On(LED3);
	...
	lcd_reg_write(SET_PIXEL_DATA_INTERFACE);  
	lcd_data_write(0x03); //0x03 is 16bit(565 format);0x00 is for 8-bit,pixeldata format
		
	lcd_reg_write(SET_LSHIFT_FREQ);   
	lcd_data_write(0x01); 
	lcd_data_write(0x99); 
	lcd_data_write(0x9a); 
...
void lcd_direction(uint8_t direction)
{
  ...
  switch(lcddev.dir)
  {
    ...
    case LCD_DISPLAY_VERTICAL:
      lcddev.width = LCD_W;
      lcddev.height = LCD_H;
      lcd_command_write(0x36, (0 << 3) | (0 << 6) | (0 << 7));
    break;
    default:break;
  }
}
